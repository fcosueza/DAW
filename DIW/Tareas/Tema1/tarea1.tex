...
\begin{itemize}
    ...
    \begin{enumerate}
        \item En primer lugar se ha modificado la \textbf{segunda imagen del carrusel}, hemos elegido otra a nuestro gusto y la hemos incluido en la web. Para llevar esto a cabo, se ha descargado la imagen deseada en la carpeta \textbf{img} proporcionada por la plantilla. Posteriormente, se ha \textbf{modificado el nombre} de la imagen en el código HTML destinado al carrusel.
        ...
        \begin{figure}[H]
            \centering
            \includegraphics[scale=0.28]{plantilla-cambio-texto.png}
            \caption{Código HTML del texto sin modificar (izquierda) y modificado (derecha)}
        \end{figure}

        \item Por último, un cambio que me ha parecido interesante realizar, es \textbf{modificar el menú de navegación} superior de la página. En este aspecto, es un cambio que es probable que se realice cuando estamos usando plantillas. En nuestro caso, se han traducido los diferentes elementos del menú al castellano y se ha eliminado el elemento \textbf{Pages}, que no lo vamos a usar.Para ello, se ha modificado el código HTML concerniente al menú tal y como podemos ver en la siguiente captura.

        \begin{figure}[H]
            \centering
            \includegraphics[scale=0.28]{plantilla-cambio-menu.png}
            \caption{Código HTML del menú sin modificar (izquierda) y modificado (derecha)}
        \end{figure}
    \end{enumerate}

    \item Por último, una vez realizados todos cambios, se muestra el resultado en la siguiente captura de pantalla, donde comparándola con la mostrada anteriormente, se puede ver con claridad todos ellos.

    \begin{figure}[H]
        \centering
        \includegraphics[scale=0.28]{plantilla-modificada.png}
        \caption{Plantilla después de la modificaciones}
    \end{figure}
\end{itemize}

...
El objetivo de esta aplicación es facilitar la \textbf{gestión de comunidades de vecinos} por parte tanto de los administradores de la propiedad como de los inquilinos o propietarios. La aplicación se ofrece de forma \textbf{gratuita} y proporciona una interfaz sencilla e intuitiva, poniendo especial énfasis en la accesibilidad y facilidad de uso. 

A lo largo de este documento, detallaremos todos los requisitos de la aplicación así como el diseño que se ha elegido y el proceso de desarrollo que va a emplear para llevar la creación de la aplicación a buen termino.

\subsection{Contexto}
Esta aplicación es un \textbf{proyecto personal} que se ha decidido desarrollar tras el análisis de diferentes aplicaciones del sector para la gestión vecinal y ver determinadas carencias, tanto en la accesibilidad como en una oferta completa de características que permite gestionar todos los aspectos de una comunidad bajo la misma aplicación.

Hay que tener en cuenta, que los usuarios objetivo son un grupo muy heterogéneo donde podemos encontrar personas con diferente formación, conocimientos sobre tecnologías o con problemas de accesibilidad, así como con dificultades económicas. Por este motivo, se ha decidido realizar una \textbf{aplicación gratuita}, centrándonos en la \textbf{accesibilidad} y \textbf{facilidad de uso}, que ayude a cualquier potencial usuario a realizar la gestión de su comunidad de forma sencilla e intuitiva. 

\subsection{Planteamiento del Problema}
La gestión de comunidades de vecinos es una tarea compleja que requiere realizar un conjunto de gestiones administrativas con la participación de los vecinos y de una forma transparente. En este aspecto, nuestra aplicación debería permitir que se realicen las siguientes tareas:

\begin{itemize}
	\item \textbf{Gestión de documentos}: permitir la inclusión de documentos relativos a la gestión de la comunidad de forma accesible y transparente para el resto de vecinos.
		
	\item \textbf{Reserva de espacios comunes}: automatizar la reserva y gestión de espacios comunes permitiendo a los vecinos reservar o cancelar reservas de dichos espacios, así como acceder a una lista con los diferentes espacios y el estado actual de reserva.
	...
	\item \textbf{Gestión de Finanzas}: la aplicación debe realizar, de forma automática, cálculos para mostrar el balance anual
	de la comunidad.
\end{itemize}

Además, hay que garantizar que estas tareas se desarrollen de una forma \textbf{segura}, empleando una \textbf{interfaz simple e intuitiva} y que sea \textbf{accesible} para todo el mundo.
...
\begin{itemize}
	...
	\item \textbf{Requerimientos de Hardware y Software}: en esta sección se especifica el hardware necesarios para realizar el desarrollo y poner en funcionamiento la aplicación así como el software que se va a necesitar para este mismo propósito. También se especifica el presupuesto y los costes de hosting para la aplicación.
	
	\item \textbf{Casos de Uso}: aquí se realiza un análisis de los principales casos de uso, empleando diferentes diagramas, así como una descripción más detalladas de los casos de uso generales.
	...
	\item \textbf{Seguridad}: por último, se van a explicar las medidas de seguridad que se van a tomar en la aplicación para
	garantizar al integridad de los datos, contraseñas, etc...
\end{itemize}
...
En esta sección se va definir la \textbf{especificación de requerimientos} que establece los requisitos funcionales y no funcionales de la aplicación SolucionesVecinales, realizando un análisis de su funcionamiento y estableciendo las restricciones para el diseño de la aplicación.

\subsubsection{Propósito}
El propósito de esta sección es \textbf{establecer los requisitos}, tanto funcionales como no funcionales, de la aplicación que se va a desarrollar, realizando un \textbf{análisis exhaustivo de su funcionamiento} y estableciendo las restricciones necesarias para que la aplicación cumpla su funcionalidad de forma correcta, segura y eficiente. Estos requisitos nos ayudarán a planificar el proceso de desarrollo así como a tomar las decisiones oportunas durante el diseño de la aplicación.

Esta sección va dirigida principalmente al \textbf{equipo de desarrollo}, que serán los encargados de diseñar la aplicación a partir de los requerimientos y restricciones establecidos en este documento, aunque teniendo en cuenta que también esta incluido en el documento del proyecto, cabe la posibilidad de que tengan acceso a él otros interesados, como por ejemplo, posibles inversores.
...
La referencias, al igual que el glosario, tanto de esta sección como del resto de secciones se incluyen al final de documento, para no duplicar secciones. Además, en cada texto que haga referencia a alguna página o documento, se incluirá la cita adecuada apuntando a dicha referencia, para facilitar su acceso.
...
\begin{itemize}
	\item \textbf{Introducción} (Sección 2.1): en este primer apartado se realiza una introducción tanto del software como de este documento, explicando su propósito, alcance, etc...
	
	\item \textbf{Descripción General} (Sección 2.2): en el segundo apartado se realiza una descripción del software que se va a desarrollar, incluyendo su funcionalidad, restricciones, características de los usuarios potenciales, interfaces, etc. Nos sirve como base para especificar los requisitos en la siguiente sección.
	
	\item \textbf{Requerimientos Específicos} (Sección 2.3): aquí vamos a establecer los requisitos específicos del software, extraídos de la descripción general y separados en requisitos funcionales y no funcionales.
	
	\item \textbf{Requerimientos de Documentación} (Sección 2.4): por último se especifican los requisitos de documentación que va a tener el software, incluyendo manuales, ayuda en línea, instalación, etc.
\end{itemize}
...
~\\\\
\-\ \-\ \-\ Debido a que las especificaciones sobre al interfaz de usuario son más extensas, se han incluido como un anexo. Estas especificaciones se pueden consultar en el \hyperref[sec:apenA]{Anexo A}.
...
En el \textbf{lado del servidor}, nuestra aplicación se ejecutará en un servidor web, aunque como hemos comentado, no se va a emplear ninguna interfaz para comunicarse con el. En concreto, nuestra aplicación se va a ejecutar en \textbf{Node}, por lo que no será necesaria ningún tipo de configuración previa salvo la instalación del propio Node.

Cabe mencionar en este apartado, que la aplicación se va a "dockerizar", es decir, se va a desplegar usando el software \textbf{Docker} para que pueda ejecutarse en cualquier servidor de forma autónoma.
t
...
~\\\\
\-\ \-\ \-\ La aplicación no va ha necesitar ningún tipo de adecuación al entorno ya que al usar Docker, más en concreto, \textbf{docker-compose}, simplemente desplegando el container la aplicación tendrá todo el software que necesita encapsulado. Si acaso, el único requisito, sería tener instalado Docker en el servidor.
...
\begin{itemize}
	\item La aplicación deberá \textbf{gestionar los diferentes usuarios} permitiendo que éstos se registren en el sistema como inquilinos o administradores y que accedan a las diferentes herramientas para gestionar su comunidad, así como a la modificación de su perfil o su eliminación. También permitirá iniciar sesión como webAdmin.
 	...
 	\item El \textbf{webAdmin} será un usuario especial que tendrá acceso al Backoffice y podrá crear, editar o eliminar los diferentes servicios que ofrece la plataforma.
...
\begin{itemize}
	\item \textbf{Front/Backend}: después de analizar las características de la aplicación y su complejidad, se ha decidido emplear
	el \textbf{Next.js} para el desarrollo del frontend y del backend. Se ha preferido respecto a React y Node ya que nos va a 
	facilitar el desarrollo de al aplicación.
	
	\item \textbf{Base de Datos}: para la base de datos se ha elegido \textbf{PostgreSQL}, ya que es una de las bases de datos más
	rápidas y flexibles, perfecta para nuestra aplicación. Como ORM nos vamos a decantar por \textbf{Prisma}, por sus sencillez y 
	por la sintaxis tan intuitiva que tiene.
	...
	También se empleará Clickup para llevar el control de las tareas.
\end{itemize}
...
\begin{itemize}
	...
	\item Se integrará \textbf{Playwright} en este proceso integración continua para la realización de los \gls{e2e}.
...
Nuestra aplicación ofrecerá una \textbf{extensa ayuda en línea} que se encargará de explicar como realizar los diferentes procedimientos que pueden llevar a cabo los usuarios en nuestra aplicación. Aún queda por determinar el formato que se empleará para mostrar esta ayuda. 																																					

\subsubsection{Guía de Instalación, Configuración y Archivo Léeme} 
Se incluirá una \textbf{guía de instalación en un entorno local} en el que se especificará como se instala el software mediante la instalación de Node y NPM, así como todas las dependencias necesarias para ejecutar al aplicación. 
...
Para el desarrollo principal de la aplicación se usará \textbf{Next.js 16} como framework de desarrollo, empleando como lenguaje de programación \textbf{Typescript}.
...
En esta sección se va incluir el diseño de la interfaz de la aplicación, especificando todas y cada una de las páginas que van a formar nuestra aplicación, así como la relación que existe entre estas. Como ya se ha comentado, hay una descripción detallada de la interfaz también incluida en el Anexo A.

\subsection{Pantallas de Login y Registro}
En estas pantallas se podrán introducir los datos de acceso, en caso de que ya se tenga un usuario, o en su defecto, se podrá registras uno nuevo introduciendo los datos requeridos.
...
\begin{figure}[H]
	\centering
	\includegraphics[scale=0.15]{registro.png}
	\caption{Página de registro}
\end{figure}

\subsection{Pantalla Principal}
La pantalla principal será la página de acceso de al aplicación, y se detalla en en el anexo ya comentado. En este apartado se incluye una captura, aunque hay que tener en cuenta que puede sufrir modificaciones durante el proceso de desarrollo.

\begin{figure}[H]
	\centering
	\includegraphics[scale=0.10]{main.png}
...
En esta sección se muestra el paso a tablas de base de datos, indicando las tablas y las relaciones que estas tienen con otras mediante flechas.  

\begin{figure}[H]
	\centering
	\includegraphics[scale=0.50]{tablas.png}
\end{figure}

Cabe destacar que el orden de algunos campos de las tablas se ha alterado, mínimamente, para que las flechas de las relaciones no se vieran muy juntas. Aún así, esto no afecta a las tablas ni a sus relaciones.

\subsection{Roles}

En la aplicación se van a definir 3 roles que nos van a ayudar a limitar los permisos que van a tener los diferentes usuarios. Los roles que se van a definir son:

\begin{itemize}
	\item \textbf{Inquilino}: los inquilinos son los usuarios con menos privilegios y los más numerosos. Este rol esta pensado para los vecinos y vecinas de una comunidad que no se encarguen de realizar tareas administrativas. El acceso que tendrán es será el siguiente:
	\begin{itemize}
		\item \textbf{Lectura}: estos usuarios podrán leer los datos de la comunidad, los mensajes y las zonas comunes en sus respectivas tablas.
		\item \textbf{Inserciones}: estos usuarios podrán leer y escribir en sus datos de usuario y en los diferentes elementos creados por sí mismos en las tablas de incidencias y reservas. No tendrán permisos para modificar los elementos creados por otros usuarios.
		\item \textbf{Borrado}: estos usuarios solo podrán borrar elementos creados por sí mismos en las tablas de incidencias y reservas, así como la actualización de sus datos de usuario.
	\end{itemize}
	
	\item \textbf{Admin}: los administradores son los usuarios encargados de la administración de la comunidad, por lo que habrá 1 por cada comunidad. Son los usuarios con más permisos dentro de una comunidad (a excepción del webAdmi). El acceso que tendrán es el siguiente: 
		\begin{itemize}
		\item \textbf{Lectura}: estos usuarios podrán leer en todas las tablas que estén relacionadas con al comunidad que gestionan.
		\item \textbf{Inserciones}: al igual que la lectura, los administradores podrán realizar inserciones en todas las tablas que estén relacionadas con la comunidad que gestionan. También podrán realizar inserciones en la tabla de comunidades para añadir nuevas comunidades de las que serán administradores.
		\item \textbf{Borrado}: los administradores podrán borrar registros de todas las tablas relacionadas con las comunidades que han creado, así como las comunidades creadas por sí mismos en al tabla comunidad.
	\end{itemize}
	
	\item \textbf{WebAdmin}: el webadmin, que se correspondería con el propietario de la aplicación, es un usuario único que tendrá acceso al \textbf{Backoffice} de la aplicación. Los permisos que tiene son:
		\begin{itemize}
			\item \textbf{Lectura, Inserción y Borrado}: este usuario podrá leer, insertar o borrar registros de cualquier tabla de la aplicación y de cualquier comunidad, lo que le permitirá gestionar cualquier servicio de la aplicación.
		\end{itemize}
\end{itemize}

Estos roles se van a \textbf{gestionar programáticamente} desde la \textbf{aplicación} y \textbf{no estarán} implementados en al\textbf{ base de datos} ya que el acceso a la base de datos desde la aplicación se va a realizar empleando un solo usuario que tendrá todos los permisos para crear, actualizar o borrar registros en cualquier tabla, y será desde la aplicación, tanto desde el frontend como desde el backend, desde donde se gestionará que pueden o no hacer los diferentes usuarios del sistema. 
...
\section{Seguridad}
En esta sección vamos a comentar las medidas de seguridad que se van a adoptar en al aplicación.
...
\label{sec:apenA}
\textbf{SolucionesVecinales} es una aplicación web, por lo que tendrá una \textbf{interfaz web} que estará compuesta por un conjunto de páginas que permitirán ofrecer los diferentes servicios de los que se compone la aplicación. Estás páginas serán accesibles desde cualquier dispositivo que tenga un navegador y es algo que se tendrá que tener en cuenta a la hora de realizar su diseño. 

Las \textbf{páginas} de las que estará compuesta nuestra interfaz son las siguientes:

\begin{itemize}
	\item \textbf{Página Principal}: está es la página de entrada a nuestra aplicación. Cualquier usuario puede acceder a ella y  su función principal es la de \textbf{ofrecer información sobre la aplicación}, exponiendo sus principales características, así como establecer un \textbf{canal de comunicación} con el equipo de desarrollo, consistente en un formulario de contacto.
	  
	Además, deberá tener un \textbf{menú} donde se incluyan enlaces a las diferentes secciones, así como a las páginas de login y registro.
	
	Las secciones de las que estará compuesta esta interfaz serán las siguientes:
	
	\begin{itemize}
		\item \textbf{Barra de Menú}: esta barra se sitúa en la parte superior de la pantalla principal y que estará siempre visible, incluirá un \textbf{logo de la aplicación} y un \textbf{menú} con enlaces a las diferentes secciones de la página principal así como a las páginas de login y registro.
		
		\item \textbf{Cabecera}: aquí se mostrará una imagen con un eslogan, el nombre de la aplicación y un elemento \gls{CTA} para que los usuarios se registren.
		...
		\item \textbf{Pie de Página}: en el pie de página se añadirá una lista de enlaces con las diferentes secciones de la página, un mapa del sitio web y enlaces a las diferentes redes sociales de la aplicación. Además se deben incluir enlaces a los documentos ó páginas con las políticas de privacidad, cookies, etc. También deberá incluir \textbf{información de copyright} sobre la página.
		
	\end{itemize}
	
	Cabe destacar que tanto el \textbf{pie de página} como la \textbf{barra de menú} son elementos comunes a todas la páginas. Dependiendo de la página en la que estemos, estos adaptarán su contenido.
	
	\item \textbf{Página de Login}: el propósito de esta página es permitir que los usuarios puedan \textbf{entrar en al aplicación}. El elemento principal de la página será un \textbf{formulario} que permitirá introducir el correo y contraseña del usuario. En caso de que los datos sean erróneos, se mostrarán mensajes de error debajo de cada campo. Además, se incluirá un \textbf{enlace a la página de registro}, para usuarios que aún no estén registrados, debajo del formulario.
	
	\item \textbf{Página de Registro}: esta página permitirá a los usuarios que no estén registrados \textbf{darse de alta} en el sistema. Constará de un formulario, donde el usuario deberá introducir los datos, así como seleccionar el rol que va a tener. En el caso de que los datos sean incorrectos, se mostrará un mensaje de error debajo de los campos afectados. Además, se incluirá un \textbf{enlace a la página de login}, debajo del formulario, para usuario que ya estén dados de alta.
	
	\item \textbf{Dashboard de Comunidad}: una vez que el usuario haya ingresado al sistema, se mostrará la pantalla de comunidad. El propósito de esta pantalla es servir como \textbf{puerta de entrada a la gestión de la comunidad} y como \textbf{punto de información} para el usuario. Esta interfaz estará compuesta por los siguientes elementos:
	
	\begin{itemize}
		\item \textbf{Barra de Menú}: similar a la que vemos en la página principal, pero las opciones de menú ``Login/Registrarse''  habrá cambiado por el \textbf{icono de un usuario}. Este icono mostrará un \textbf{menú vertical} con las opciones ``\textbf{Perfil}'' y ``\textbf{Logout}'', que permitirán acceder a la información de perfil y hacer \gls{logout} respectivamente. Es probable que se añadan más opciones durante el proceso de desarrollo.
...
\textbf{NOTA}: cabe destacar que los diseños son susceptibles de pequeños (o no tan pequeños) cambios durante el proceso de desarrollo, ya sea para mejorar al experiencia de usuario, por razones de accesibilidad o por cambios en el diseño de la interfaz
en general.

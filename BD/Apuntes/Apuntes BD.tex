...
\section{Instalación y Primeros Pasos en MySQL Server}
\textbf{MySQL} es un SGBD (DBMS por sus siglas en inglés) muy conocido y ampliamente usado por su \textbf{simplicidad} y \textbf{notable rendimiento}. Se ofrece libre bajo licencia GNU GPL. Esto le otorga como beneficios gran estabilidad y un desarrollo rápido. Además esta disponible para múltiples plataformas.

Entre las principales \textbf{características} que tiene MySQL son las siguientes:

\begin{itemize}
    \item Esta desarrollado en C++.
    \item Se distribuye en ejecutable para más de 19 plataformas.
    \item La API se encuentra disponible en C, C++, Eiffel, Java, Perl, PHP, Python, Ruby y TCL.
    \item Esta optimizado para equipos con múltiples procesadores.
    \item Tiene una velocidad de respuesta muy alta.
    \item Se puede utilizar como cliente/servidor o incrustado en aplicaciones.
    \item Cuenta con un rico conjunto de tipos de datos.
    \item Soporta diferentes métodos de almacenamiento de tablas, con prestaciones y rendimiento diferente para poder optimizar el SGBD para cada caso concreto.
\end{itemize}
